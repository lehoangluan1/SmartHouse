...
def has_server_error():
  return (
    SYS.get('state_error') or
    SYS.get('config_error') or
    SYS.get('command_error') or
    SYS.get('registry_error')
  )
...
def sensor_is_valid():
  t, h, l = SYS.get('nhiet_do'), SYS.get('do_am'), SYS.get('shine')
...
def send_next_telemetry():
  ...
  for _ in range(total):
    ...
    if device_key is None:
      print("SKIP: no device_key", key_name)
      continue

    if value is None:
      print("SKIP: no value", sys_key)
      continue

    print("SEND TELEMETRY:", sensor_type, value)

    return send_one_telemetry(device_key, sensor_type, value)

  print("NO VALID TELEMETRY DATA")
  return False
...
while True:
  ...
  # ===== TEMP ALERT =====
  if SYS.get('nhiet_do') is not None and SYS['nhiet_do'] > CFG['Tcritical']:
  ...
  # ===== ADAPT INTERVAL =====
  if has_server_error():
    intervals['command'] = 3000
    intervals['state'] = 5000
    intervals['telemetry'] = 3000
  else:
    intervals['command'] = 3000
    intervals['state'] = 2000
    intervals['telemetry'] = 5000
  ...
  if time.ticks_diff(now, last['telemetry']) >= intervals['telemetry']:
    ...
    if sensor_is_valid():
      success = send_next_telemetry()
      SYS['telemetry_error'] = not success

      if not success:
        print("RETRY TELEMETRY")
        time.sleep_ms(200)
        send_next_telemetry()
    else:
      print("SKIP: sensor invalid")
